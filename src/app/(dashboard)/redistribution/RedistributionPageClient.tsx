...
export function RedistributionPageClient({ tenantId: tenantIdProp }: { tenantId?: string }) {
  ...
  const totalPages = Math.max(1, Math.ceil(sortedRecommendations.length / PAGE_SIZE));
  const safePage = Math.min(currentPage, totalPages);
  const pageStart = (safePage - 1) * PAGE_SIZE;
  const pageEnd = Math.min(pageStart + PAGE_SIZE, sortedRecommendations.length);
  const pagedRecommendations = sortedRecommendations.slice(pageStart, pageEnd);
        ...
        <div className="rounded-2xl border border-border bg-card">
          <div className="flex flex-wrap items-start justify-between gap-3 border-b border-border px-4 py-3">
            <div>
              <div className="flex items-center gap-2 text-sm font-semibold text-foreground">
                ...
                Что нужно сделать ({sortedRecommendations.length}{sortedRecommendations.length === 1 ? ' перемещение' : ' перемещений'})
              </div>
              <div className="mt-1 text-xs text-muted-foreground">
                Отсортировано по выгоде. Размеры и количество указаны явно —
                это те же данные, что попадут в Excel.
              </div>
            </div>
            {totalPages > 1 && (
              <div className="text-xs text-muted-foreground tabular-nums">
                Показаны <span className="font-semibold text-foreground">{pageStart + 1}–{pageEnd}</span> из {sortedRecommendations.length}
              </div>
            )}
          </div>
          <div className="grid grid-cols-1 gap-3 p-4">
            {pagedRecommendations.map((rec, idx) => (
              <RecommendationCard
                key={`${rec.nmId}-${rec.sizeName}-${rec.fromOfficeId ?? rec.fromWarehouse}-${rec.toOfficeId ?? rec.toWarehouse}-${pageStart + idx}`}
                rec={rec}
