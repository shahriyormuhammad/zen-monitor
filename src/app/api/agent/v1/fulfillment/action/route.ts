...
import {
  createFulfillmentStockApprovalRequest,
  PROCIFRY_FULFILLMENT_ACTION_TYPES,
  type ProcifryFulfillmentActionType,
} from '@/server/agent/procifry-fulfillment';
...
const actionTypeSchema = z.enum(PROCIFRY_FULFILLMENT_ACTION_TYPES);
...
const bodySchema = z.object({
  ...
  ownStockItems: z.array(z.record(z.string(), z.unknown())).max(1000).optional(),
  productionOrders: z.array(z.record(z.string(), z.unknown())).max(500).optional(),
});
...
function buildActionPayload(raw: z.infer<typeof bodySchema>) {
  ...
  return {
    ...
    ownStockItems: raw.ownStockItems,
    productionOrders: raw.productionOrders,
  };
}

function sourceFrom(raw: z.infer<typeof bodySchema>) {
  return raw.source?.trim() || 'procifry-agent';
}
...
function normalizeBody(raw: z.infer<typeof bodySchema>, clientWorkerId: ProcifryWorkerId | null) {
  ...
  return {
    ...
    actionType: (raw.action_type ?? raw.actionType ?? 'fulfillment_stock_update') as ProcifryFulfillmentActionType,
    payload,
  };
}
...
export const POST = withRateLimit(apiRoute(async (request: Request) => {
  ...
  const result = await createFulfillmentStockApprovalRequest(client, payload);
